...
fun VentanaPerfil(navController: NavHostController) {
    // Variable que guarda la opción seleccionada en el menú
    val seleccionMenu = remember { mutableStateOf(2) }

    // Autenticación de Firebase y usuario actual
    val auth = FirebaseAuth.getInstance()
    val user: FirebaseUser? = auth.currentUser

    // Variables para almacenar nombre de usuario y foto de perfil
    ...
    Scaffold(
        bottomBar = {
            BottomNavigationBar(navController) // Uso la barra de navegación
        }
    ) { padding ->
        ...
        Box(
            ...
            Column(
                ...
                // Imagen de perfil (si no se ha seleccionado, se muestra la foto de perfil actual o un ícono por defecto)
                Image(
                ...
                // Correo del usuario
                Text(
                    text = user?.email ?: "usuario@correo.com",
                    fontSize = 16.sp,
                ...
                ClickableText(
                    ...
                    onClick = {
                        ...
                        navController.navigate("editProfile") // pantalla para editar el perfil
                    }
                )
            }
        }
